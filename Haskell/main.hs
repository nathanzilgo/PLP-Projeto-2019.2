...
attack atacante atacado estado1 estado2
    | (getEstado atacante estado1) > 1 && (getEstado atacado estado2) > 0 = 
        ...
        if(dado1 <= dado2)
            then 
                ...
                atacante = removeTroops (atacante 1 estado1)
        else
            then 
                exibeJogada( (name atacado) ++ " perde a disputa com valor " ++ dado2 ++ " nos dados, contra " ++ dado1)
                atacado = removeTroops (atacado 1 estado2)
        
        attack atacante atacado estado1 estado2

    | otherwise = atacante atacado
